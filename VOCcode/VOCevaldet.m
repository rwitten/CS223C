...
imagestoload=VOCopts.TEST_IMAGES;
% load test set
...
for i=1:imagestoload
    ...
    gt(i).image = imread(sprintf(VOCopts.imgpath,gtids{i}));
    drawWithBB(gt(i).image,gt(i).BB,sprintf('realimage%d.png', i));
...
fprintf('this is the length of what we are iterating over %d\n', nd);
tp=zeros(nd,1);
fp=zeros(nd,1);
tic;
for d=1:nd
    ...
    if ovmax>=VOCopts.minoverlap
        if ~gt(i).diff(jmax)
            if ~gt(i).det(jmax)
                disp 'YAY A TRUE POSITIVE'
                imwrite(gt(i).image, sprintf('TRUEPOSITIVE%d.png', i), 'png');
                tp(d)=1;            % true positive
		gt(i).det(jmax)=true;
            else
                disp 'WEIRD FALSE POSITIVE'
